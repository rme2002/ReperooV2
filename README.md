...
1. **Set the context** – Ask Codex to read `openspec/project.md` (see the sample prompt generated by `openspec init`). Keep this file up to date whenever tech stack or conventions change.
2. **Decide if a spec is required** – Use OpenSpec for net-new capabilities, behaviour changes (API contracts, UX, data models), architectural shifts, or performance/security work. Skip it for pure bug fixes, typo/doc/config tweaks, dependency bumps, or tests that simply cover existing behaviour.
3. **Request a proposal** – When you need new functionality, tell Codex what you want (e.g., “I want to add team invites. Please create an OpenSpec change proposal for this feature”). The agent will pick a change ID, scaffold `proposal.md`/`tasks.md`, and draft spec deltas for you.
4. **Review & refine** – If the proposal isn’t perfect, ask Codex to revise specific sections (“update the scenarios to cover passwordless login”, “split the tasks by surface”). Iterate until the spec captures the desired behavior, then approve it.
5. **Wait for approval before coding** – Once the proposal looks good, give the green light and ask Codex to implement the approved tasks. Codex will follow the checklist, run validations/tests, and keep the spec + code in sync.
6. **Wrap up** – After deployment, ask Codex to archive the change (`openspec archive <id> --yes`) so the canonical specs under `openspec/specs/` stay current.
