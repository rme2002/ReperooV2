...
import { incomeTypeOptions, type IncomeEvent } from "@/src/features/budget/types";
...
export function AddIncomeModal({
  ...
  const { addIncome, updateIncome, deleteIncome } = useBudgetContext();
  useEffect(() => {
  ...
  const isFormValid = isAmountValid && isTypeValid && !isFutureDate;

  const monthDays = useMemo(() => getMonthDays(monthCursor), [monthCursor]);
  const atCurrentMonth = monthCursor.getFullYear() === today.getFullYear() && monthCursor.getMonth() === today.getMonth();
  ...
  const handleSubmit = () => {
    ...
    if (mode === "edit" && initialIncome) {
      updateIncome(monthKey, initialIncome.id, payload);
      showToast("Income updated");
    } else {
      addIncome(monthKey, payload);
  ...
  const handleDelete = () => {
    ...
    Alert.alert("Delete income", "Remove this income entry?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          deleteIncome(monthKey, initialIncome.id);
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.sheet}>
          ...
          >
            ...
            {isFutureDate ? <Text style={styles.helperText}>No future dates</Text> : null}
          </View>
          ...
          <View style={styles.recurringSurface}>
            ...
            <Text style={styles.recurringCopy}>Repeat this income every month.</Text>
            {isRecurring ? (
          ...
          {mode === "view" ? (
            ...
            >
              <Text style={styles.saveText}>{mode === "edit" ? "Save changes" : "Save income"}</Text>
            </Pressable>
          )}
        </View>
          ...
          <Pressable style={styles.calendarOverlay} onPress={() => setShowCalendar(false)}>
            <Pressable style={styles.calendarModal} onPress={(e) => e.stopPropagation()}>
              <View style={styles.calendarHeader}>
                ...
                >
                  <Text
                    style={[styles.calendarNavText, atCurrentMonth && styles.calendarNavTextDisabled]}
                  >
                    ›
                  </Text>
                </Pressable>
