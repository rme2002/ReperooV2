...
import { recurringExpensesSeed } from "@/components/dummy_data/recurringExpenses";
import { ledgerMonths } from "@/components/dummy_data/transactions";
...
} from "@/components/transactions/recurring";
...
export default function TransactionsScreen() {
  ...
  const monthTransactions = activeMonth?.transactions ?? [];
  ...
  const filteredTransactions = useMemo(() => {
    ...
    const filtered = base.filter((tx) => {
      ...
      const categoryLabel = getCategoryLabel(tx.categoryId).toLowerCase();
      const subLabel = (getSubcategoryLabel(tx.categoryId, tx.subcategoryId) ?? "").toLowerCase();
      const note = (tx.note ?? "").toLowerCase();
      return categoryLabel.includes(query) || subLabel.includes(query) || note.includes(query);
    });
  ...
  const confirmDelete = (txId: string) => {
    const expense = activeMonth?.transactions.find((tx) => tx.id === txId) ?? null;
  ...
  const renderTransactionRow = ({ item, section }: { item: TransactionEntry; section: TransactionSection }) => {
                  ...
                  return (
                    <View style={styles.sectionHeader}>
                      <View style={styles.sectionHeaderRow}>
                        ...
                        <Text style={styles.sectionHeaderValue}>{formatCurrency(section.total)}</Text>
                      </View>
                      {total > 0 ? (
