...
3. Principal: the Workload Identity pool service account (`service-<PROJECT_NUMBER>@gcp-sa-iam.iam.gserviceaccount.com`) or simply add **Workload Identity User** with the condition auto-generated by the UI.
4. Role: `Workload Identity User`.

This connects the WIF provider to the CI service account so the workflow can request temporary tokens without JSON keys.
