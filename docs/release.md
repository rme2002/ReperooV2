...
- `apps/mobile/app.config.js` drives the Expo config. It reads `APP_VARIANT` (from EAS profiles or CI) to flip the display name/icon suffixes and bundle identifiers (`com.rjaay23.startermono` for prod vs `com.rjaay23.startermono.dev` for dev). Update the `baseId`, `name`, `owner`, icon paths, and the `EXPO_EAS_PROJECT_ID` entry in `apps/mobile/eas.json` (which feeds `extra.eas.projectId`) to your Expo project before publishing.
